...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=True, two_stage_num_proposals=300,
        ...
        self.multiscale = MultiScaleFeature(is_5_scale=False)
        self.cgfe = CGFE(gate_channels=288, reduction_ratio=16, num_feature_levels=self.num_feature_levels)
        self.dynamic_query_list = [300, 500, 900, 1500]
    ...
    def build_query_attn_mask(self, num_track, num_detect, device):
        """
        构造一个 Nq x Nq 的布尔mask
        True表示masked（不可见），False表示可见。

        - track queries在后期层数可以互相mask（cascaded）
        - 这里先给个最简单版的示例：让track queries互相不可见
        """
        N = num_track + num_detect
        attn_mask = torch.zeros((N, N), dtype=torch.bool, device=device)

        # 让track queries之间互相不可见（只看自己）
        for i in range(num_track):
            for j in range(num_track):
                if i != j:
                    attn_mask[i, j] = True

        # track queries 和 detection queries 是可见的
        # detection queries之间是可见的
        # 所以不用改其他位置
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, targets=None):
        ...
        # print('ENTER Def.DETR!!!')
        # print("srcs 的形状:")
        # for i, src in enumerate(srcs):
        #     print(f"srcs[{i}].shape: {src.shape}")
        # print("masks 的形状:")
        # for i, mask in enumerate(masks):
        #     print(f"masks[{i}].shape: {mask.shape}")
        ...
        prev_counting_output, prev_ccm_feature = self.ccm(prev_memory, spatial_shapes)
        counting_output, ccm_feature = self.ccm(memory, spatial_shapes)
        # print(counting_output)
        prev_multi_ccm_feature = self.multiscale(prev_ccm_feature)
        multi_ccm_feature = self.multiscale(ccm_feature)
        multi_ccm_feature = multi_ccm_feature + prev_multi_ccm_feature
        # print("len prev ccm feature:", len(prev_multi_ccm_feature))
        ...
        # max_predicted = max(predicted.tolist())
        # if max_predicted >= len(self.dynamic_query_list):
        #     num_select = self.dynamic_query_list[-1]  # 使用最后一个值作为默认值
        #     print("max_predicted >= len(self.dynamic_query_list)")
        # else:
        #     num_select = self.dynamic_query_list[max_predicted]
        ...
        if self.two_stage:
            ...
            query_embed = query_embed[:num_select].unsqueeze(0).expand(bs, -1, -1)
            tgt = tgt[:num_select].unsqueeze(0).expand(bs, -1, -1)
            ...
            if targets is not None and 'track_query_hs_embeds' in targets[0]:
                ...
                prev_query_embed = prev_query_embed[:, :num_select]  # 使用 num_select
                prev_tgt = prev_tgt[:, :num_select]  # 使用 num_select
                ...
                # num_track_queries = prev_query_embed.shape[1]
                # num_detect_queries = query_embed.shape[1] - num_track_queries  # 因为拼接后是总数
                #
                # query_attn_mask = self.build_query_attn_mask(
                #     num_track_queries,
                #     num_detect_queries,
                ...
                #     print("Using track_queries_placeholder_mask from targets")
