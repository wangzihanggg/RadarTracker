...
from RadarTracker.datasets.tracking import TrackDatasetFactory
from RadarTracker.models import build_model
from RadarTracker.models.tracker import Tracker
from RadarTracker.util.misc import nested_dict_to_namespace
from RadarTracker.util.track_utils import (evaluate_mot_accums, get_mot_accum,
...
def main(seed, dataset_name, obj_detect_checkpoint_file, tracker_cfg,
    ...
    if obj_detector_model is None:
        _log.info(f"EVAL:")
        print(s.no_gt for s in dataset)
        print([str(s) for s in dataset if not s.no_gt])
