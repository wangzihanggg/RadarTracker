...
class CGFE(nn.Module):
    ...
    def forward(self, x, memory, spatial_shapes):
        ...
        # print("memory shape: ", encoder_feat.shape)
        bs, c, hw = encoder_feat.shape
        # print('x_shape:', x.shape)
        # print('x_shape:', len(x))  # 现在可以调用 .shape 属性
        ...
        for i in range(self.num_feat):
            ...
            feat = encoder_feat[:, :, idx:idx + h * w].view(bs, 288, h, w)
            # print(idx)
            # print(feat.shape)
            # print(i)
            # print(feat.shape)
...
class MultiScaleFeature(nn.Module):
    def __init__(self, channels=288, is_5_scale=False):
